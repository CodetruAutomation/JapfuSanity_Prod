...
import com.Japfu.enums.FailureHandling;
...
import com.Japfu.utils.DateUtils;
import com.Japfu.utils.LogUtils;
//import com.Japfu.utils.SoundUtils;
...
//	}
	...
	@Step("Verify element visible {0}")
	public static boolean waitElementDisplayed(By by) {
		//smartWait();
...
//		smartWait();
	...
	@Step("Set text on text box using JavaScript")
	public static void setTextUsingJS(By by, String value) {
	    WebElement element = waitForElementVisible(by);
	    
	    // JavaScript execution to set text
	    JavascriptExecutor js = (JavascriptExecutor) DriverManager.getDriver();
	    js.executeScript("arguments[0].value = arguments[1];", element, value);
	    
	    LogUtils.info("Set text " + value + " on " + by.toString() + " using JavaScript");
	    
	    if (ExtentTestManager.getExtentTest() != null) {
	        ExtentReportManager.pass("Set text " + value + " on " + by.toString() + " using JavaScript");
	    }
	    
	    AllureManager.saveTextLog("Set text " + value + " on " + by.toString() + " using JavaScript");
	...
	@Step("Set text on text box")
	public static void setCharText(By by, CharSequence[] value) {
		waitForElementVisible(by).sendKeys(value);
		LogUtils.info("Set text " + value + " on " + by.toString());
		//  playDefaultSound();
		if (ExtentTestManager.getExtentTest() != null) {
	...
	@Step("Set text on text box and press key")
	public static void sendKeys_perform(Keys keys) {
		Actions actions = new Actions(DriverManager.getDriver());
	...
	public static void KeyBoardPress_ClearField(By by) {
		DriverManager.getDriver().findElement(by).sendKeys(Keys.chord(Keys.CONTROL,"A"), Keys.chord(Keys.BACK_SPACE));
		LogUtils.info("Press key " + "ctrl+A" + " on keyboard");

		if (ExtentTestManager.getExtentTest() != null) {
			ExtentReportManager.pass("Press key " + "ctrl+A"+ " on keyboard");
		}
		AllureManager.saveTextLog("Press key " +"ctrl+A" + " on keyboard");

		addScreenshotToReport(Thread.currentThread().getStackTrace()[1].getMethodName() + "_" + DateUtils.getCurrentDateTime());
	}

	public static void KeyBoardPress_BackSpace(By by) {
		DriverManager.getDriver().findElement(by).sendKeys(Keys.chord(Keys.BACK_SPACE));
		LogUtils.info("Press key " + "ctrl+A" + " on keyboard");
...
//	}
	...
	public static void waitForPageLoaded() {
		WebDriverWait wait = new WebDriverWait(DriverManager.getDriver(), Duration.ofSeconds(FrameworkConstants.WAIT_PAGE_LOADED), Duration.ofMillis(5000));
		JavascriptExecutor js = (JavascriptExecutor) DriverManager.getDriver();
	...
	@Step("Get text of element {0}")
	public static String printStatement(String question, String statement ) {
		smartWait();
		AllureManager.saveTextLog("Message - " + statement);
		AllureManager.saveTextLog("==> The Message is: " + statement);
		System.out.println(question+ statement);
		if (ExtentTestManager.getExtentTest() != null) {
			ExtentReportManager.pass(question+ statement);
		}
		return statement;

	}

	@Step("Scroll to down full page {0}")
	...
	public static void takeFullPageScreenshotofExcel(String screenName) {
		//SimpleDateFormat dateFormat = new SimpleDateFormat("dd-MM-yyyy HH-mm-ss");
		try {
			String path = Helpers.getCurrentDir() + FrameworkConstants.EXPORT_CAPTURE_EXCEL_PATH;
			Rectangle captureRect = new Rectangle(0, 0, 1200, 800); // Coordinates and size
			Robot robot = new Robot();
			BufferedImage screenImage = robot.createScreenCapture(captureRect);

			// Save the screenshot to a file
			File screenshotFile = new File(path+"/"+screenName+".png");
	...
	public static void takeFullPageScreenshotWithoutDate(String screenName) {
		//SimpleDateFormat dateFormat = new SimpleDateFormat("dd-MM-yyyy HH-mm-ss");
		try {
			String path =Helpers.getCurrentDir() + FrameworkConstants.EXPORT_CAPTURE_EXCEL_PATH;
			File file = new File(path);
			...
			File source = ts.getScreenshotAs(OutputType.FILE);
			// result.getName() gets the name of the test case and assigns it to the screenshot file name
			FileUtils.copyFile(source, new File(path + "/" + screenName + ".png"));
			LogUtils.info("Screenshot taken: " + screenName);
			LogUtils.info("Screenshot taken current URL: " + getCurrentUrl());
		} catch (Exception e) {
			System.out.println("Exception while taking screenshot: " + e.getMessage());
	...
	@Step("Scroll to Top full page {0}")
